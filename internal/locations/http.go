...
func(h *Handler)createPlace(w http.ResponseWriter,r *http.Request){id,ok:=userID(r);if !ok{unauthorized(w);return};var in SavePlaceInput;if err:=decode(w,r,&in);err!=nil{bad(w);return};v,err:=h.service.CreatePlace(r.Context(),id,in);if err!=nil{locationError(w,err);return};httpserver.WriteJSON(w,http.StatusCreated,v)}
