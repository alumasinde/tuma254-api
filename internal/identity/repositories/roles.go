...
func (r *Postgres) assignRole(ctx context.Context, tx pgx.Tx, userID uuid.UUID, roleName string) error {
	_, err := tx.Exec(ctx, "INSERT INTO user_roles(user_id,role_id) SELECT $1,id FROM roles WHERE name=$2 ON CONFLICT DO NOTHING", userID, roleName)
	return err
}

func (r *Postgres) AssignRole(ctx context.Context, userID uuid.UUID, roleName string) error {
	tx, err := r.db.Begin(ctx)
	if err != nil {
		return err
	}
	defer tx.Rollback(ctx)
	if err := r.assignRole(ctx, tx, userID, roleName); err != nil {
		return err
	}
