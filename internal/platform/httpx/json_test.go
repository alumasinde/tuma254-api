...
func TestDecodeJSONRejectsTrailingValue(t *testing.T) {
	...
	if err := DecodeJSON(rec, req, &dst); err == nil { t.Fatal("expected trailing JSON value to fail") }
}
...
func TestWithRequestID(t *testing.T) {
	...
	if got == "" || rec.Header().Get("X-Request-ID") != got { t.Fatal("expected request id in context and response") }
}
