...
func Load()(Config,error){
 ...
 if env!="development"&&env!="test"&&env!="staging"&&env!="production"{return Config{},errors.New("APP_ENV must be development, test, staging, or production")}
 if env=="production"{
  lower:=strings.ToLower(cfg.JWTSigningKey)
  if strings.Contains(lower,"change-me")||strings.Contains(lower,"example")||strings.Contains(lower,"development")||strings.Contains(lower,"secret"){
   return Config{},errors.New("JWT_SIGNING_KEY must not use a default or development value in production")
  }
  if strings.Contains(strings.ToLower(cfg.MongoDBURI),"localhost")||strings.Contains(strings.ToLower(cfg.MongoDBURI),"127.0.0.1"){
   return Config{},errors.New("MONGODB_URI must not point to localhost in production")
  }
 }
 return cfg,nil
}
