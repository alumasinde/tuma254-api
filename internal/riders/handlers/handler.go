...
func (h *Handler) GetMe(w http.ResponseWriter, r *http.Request) {
	...
	if errors.Is(err, services.ErrInvalidState) {
		httpx.Error(w, 409, "invalid rider state")
		return
	}
	if err != nil {
		httpx.Error(w, 404, "rider profile not found")
		return
	}
...
func (h *Handler) Create(w http.ResponseWriter, r *http.Request) {
	...
	profile, err := h.svc.CreateApplication(r.Context(), userID)
	if err != nil {
		httpx.Error(w, 400, "rider application failed")
		return
	}
...
func (h *Handler) Submit(w http.ResponseWriter, r *http.Request) {
	...
	profile, err := h.svc.SubmitApplication(r.Context(), userID)
	if err != nil {
...
func (h *Handler) SetAvailability(w http.ResponseWriter, r *http.Request) {
	...
	profile, err := h.svc.SetAvailability(r.Context(), userID, in.Availability)
	if err != nil {
		httpx.Error(w, 409, err.Error())
		return
	}
...
func (h *Handler) AddVehicle(w http.ResponseWriter, r *http.Request) {
	...
	vehicle, err := h.svc.AddVehicle(r.Context(), userID, in)
	if err != nil {
...
func (h *Handler) ListVehicles(w http.ResponseWriter, r *http.Request) {
	...
	vehicles, err := h.svc.ListVehicles(r.Context(), userID)
	if err != nil {
		httpx.Error(w, 404, "rider profile not found")
		return
	}
...
func (h *Handler) SetActiveVehicle(w http.ResponseWriter, r *http.Request) {
	...
	vehicleID, err := uuid.Parse(r.PathValue("vehicleID"))
	if err != nil {
		httpx.Error(w, 400, "invalid vehicle id")
		return
	}
	vehicle, err := h.svc.SetActiveVehicle(r.Context(), userID, vehicleID)
	if err != nil {
		httpx.Error(w, 409, err.Error())
		return
	}
