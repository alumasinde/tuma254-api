...
func (r *Repository) FindProfile(ctx context.Context, userID uuid.UUID) (models.Profile, error) {
	return scanProfile(r.db.QueryRow(ctx, `SELECT id,user_id,verification_status,availability,COALESCE(rejection_reason,''),created_at,updated_at FROM rider_profiles WHERE user_id=$1`, userID))
}

func (r *Repository) CreateProfile(ctx context.Context, userID uuid.UUID) (models.Profile, error) {
	return scanProfile(r.db.QueryRow(ctx, `INSERT INTO rider_profiles(user_id) VALUES($1) RETURNING id,user_id,verification_status,availability,COALESCE(rejection_reason,''),created_at,updated_at`, userID))
}
...
func (r *Repository) UpdateProfile(ctx context.Context, userID uuid.UUID, status models.VerificationStatus, availability models.Availability, reason string) (models.Profile, error) {
	return scanProfile(r.db.QueryRow(ctx, `UPDATE rider_profiles SET verification_status=$2,availability=$3,rejection_reason=NULLIF($4,''),updated_at=now() WHERE user_id=$1 RETURNING id,user_id,verification_status,availability,COALESCE(rejection_reason,''),created_at,updated_at`, userID, status, availability, reason))
}

func (r *Repository) HasActiveVehicle(ctx context.Context, riderID uuid.UUID) (bool, error) {
	var ok bool
	err := r.db.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM rider_vehicles WHERE rider_id=$1 AND active)`, riderID).Scan(&ok)
	return ok, err
}

func (r *Repository) CreateVehicle(ctx context.Context, v models.Vehicle) (models.Vehicle, error) {
	return scanVehicle(r.db.QueryRow(ctx, `INSERT INTO rider_vehicles(rider_id,vehicle_type,registration_number,make,model,color,active) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING id,rider_id,vehicle_type,registration_number,COALESCE(make,''),COALESCE(model,''),COALESCE(color,''),active,created_at,updated_at`, v.RiderID, v.Type, v.RegistrationNumber, v.Make, v.Model, v.Color, v.Active))
}
