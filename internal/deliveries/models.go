...
type CustodyEvidence struct{Kind string `bson:"kind"`;Reference string `bson:"reference,omitempty"`;Note string `bson:"note,omitempty"`;CapturedAt time.Time `bson:"capturedAt"`;ActorID bson.ObjectID `bson:"actorId"`;Location locations.Point `bson:"location"`}
type CustodyEvent struct{ID bson.ObjectID `bson:"_id,omitempty"`;DeliveryID bson.ObjectID `bson:"deliveryId"`;Stage string `bson:"stage"`;FromParty string `bson:"fromParty"`;ToParty string `bson:"toParty"`;ActorID bson.ObjectID `bson:"actorId"`;Location locations.Point `bson:"location"`;Evidence CustodyEvidence `bson:"evidence"`;At time.Time `bson:"at"`}
type CustodyInput struct{Kind string `json:"kind"`;Reference string `json:"reference,omitempty"`;Note string `json:"note,omitempty"`;Location locations.Point `json:"location"`}
