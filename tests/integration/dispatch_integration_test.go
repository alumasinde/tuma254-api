...
func seedDelivery(t *testing.T, r *deliveries.Repository, id, sender bson.ObjectID) deliveries.Delivery {
	...
	d := deliveries.Delivery{
		ID: id, Sender: deliveries.Party{UserID: sender}, Method: deliveries.MethodNearby, Status: deliveries.StatusRequested,
		...
		Package: deliveries.Package{Description: "Parcel", WeightKG: 1}, CreatedAt: now, UpdatedAt: now,
	}
	if err := r.Create(context.Background(), d); err != nil { t.Fatal(err) }
	return d
}

func TestDispatchConcurrentAcceptOnlyOneWins(t *testing.T) {
	x := dispatchDB(t); ctx := context.Background(); sender := bson.NewObjectID(); d := seedDelivery(t, x.repo, bson.NewObjectID(), sender)
	rider := bson.NewObjectID(); now := time.Now().UTC()
	...
	elig := &dispatchEligibility{calls: map[string]int{}, answers: map[string][]bool{rider.Hex(): {true}}}; s := deliveries.NewDispatchService(x.repo, x.offers, x.plans, elig, dispatchNearby{})
	const attempts = 2; var wg sync.WaitGroup; wg.Add(attempts); results := make(chan error, attempts)
	for i := 0; i < attempts; i++ { go func(){ defer wg.Done(); _, err := s.Accept(ctx, o.ID.Hex(), rider.Hex()); results <- err }() }
	wg.Wait(); close(results)
	wins := 0; for err := range results { if err == nil { wins++ } }
	if wins != 1 { t.Fatalf("expected exactly one winner, got %d", wins) }
	stored, err := x.repo.Find(ctx, d.ID); if err != nil { t.Fatal(err) }
	if stored.Status != deliveries.StatusAssigned || stored.RiderID == nil || *stored.RiderID != rider { t.Fatalf("delivery not assigned correctly: %+v", stored) }
}
...
func TestDispatchDeclineSkipsRiderWhoBecomesUnavailable(t *testing.T) {
	...
	res, err := s.StartNearby(ctx, d.ID.Hex(), sender.Hex(), deliveries.NearbyDispatchInput{}); if err != nil { t.Fatal(err) }
	if res.RiderID != r1.Hex() { t.Fatalf("expected first offer rider1, got %s", res.RiderID) }
	if err := s.Decline(ctx, res.OfferID, r1.Hex()); err != nil { t.Fatal(err) }
	plan, err := x.plans.FindByDelivery(ctx, d.ID); if err != nil { t.Fatal(err) }
	if plan.ActiveOfferID == nil { t.Fatal("expected next active offer") }
	offer, err := x.offers.Find(ctx, *plan.ActiveOfferID); if err != nil { t.Fatal(err) }
	if offer.RiderID != r3 { t.Fatalf("expected unavailable rider2 skipped and rider3 offered, got %s", offer.RiderID.Hex()) }
}

func TestDispatchExpiryRaceAdvancesOnce(t *testing.T) {
	x := dispatchDB(t); ctx := context.Background(); sender := bson.NewObjectID(); d := seedDelivery(t, x.repo, bson.NewObjectID(), sender)
	r1, r2 := bson.NewObjectID(), bson.NewObjectID(); now := time.Now().UTC()
	elig := &dispatchEligibility{calls: map[string]int{}, answers: map[string][]bool{r2.Hex(): {true}}}; s := deliveries.NewDispatchService(x.repo, x.offers, x.plans, elig, dispatchNearby{})
	...
	if err := x.plans.Create(ctx, p); err != nil { t.Fatal(err) }
	var wg sync.WaitGroup; wg.Add(2); errs := make(chan error, 2)
	for i := 0; i < 2; i++ { go func(){ defer wg.Done(); errs <- s.ExpireAndAdvance(ctx, o.ID.Hex()) }() }
	wg.Wait(); close(errs)
	plan, err := x.plans.FindByDelivery(ctx, d.ID); if err != nil { t.Fatal(err) }
	if plan.ActiveOfferID == nil { t.Fatal("expected one next offer") }
	offer, err := x.offers.Find(ctx, *plan.ActiveOfferID); if err != nil { t.Fatal(err) }
	if offer.RiderID != r2 { t.Fatalf("expected rider2, got %s", offer.RiderID.Hex()) }
}

func TestDispatchAcceptanceStopsExpiredOfferReassignment(t *testing.T) {
	x := dispatchDB(t); ctx := context.Background(); sender := bson.NewObjectID(); d := seedDelivery(t, x.repo, bson.NewObjectID(), sender)
	r1 := bson.NewObjectID(); now := time.Now().UTC(); elig := &dispatchEligibility{calls: map[string]int{}, answers: map[string][]bool{r1.Hex(): {true}}}; s := deliveries.NewDispatchService(x.repo, x.offers, x.plans, elig, dispatchNearby{})
	...
	if err := x.offers.Create(ctx, o); err != nil { t.Fatal(err) }
	if _, err := s.Accept(ctx, o.ID.Hex(), r1.Hex()); err != nil { t.Fatal(err) }
	if err := s.ExpireAndAdvance(ctx, o.ID.Hex()); err == nil { t.Fatal("expected non-expired offer to be rejected") }
	stored, err := x.repo.Find(ctx, d.ID); if err != nil { t.Fatal(err) }
	if stored.RiderID == nil || *stored.RiderID != r1 { t.Fatal("assignment changed unexpectedly") }
}
